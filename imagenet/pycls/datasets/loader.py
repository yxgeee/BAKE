...
from pycls.datasets.sampler import DistributedClassSampler
...
def _construct_loader(dataset_name, split, batch_size, shuffle, drop_last):
    ...
    if (cfg.TRAIN.INTRA_IMGS > 0 and split == cfg.TRAIN.SPLIT):
        sampler = DistributedClassSampler(dataset._imdb, cfg.TRAIN.INTRA_IMGS+1)
    else:
...
def shuffle(loader, cur_epoch):
    ...
    assert isinstance(loader.sampler, (RandomSampler, DistributedSampler, DistributedClassSampler)), err_str
    # RandomSampler handles shuffling automatically
    if isinstance(loader.sampler, (DistributedSampler, DistributedClassSampler)):
