...
def load_dataset(name, root, sample='default', **kwargs):
    ...
    if name in ['imagenet','tinyimagenet', 'CUB200', 'STANFORD120', 'MIT67']:
        ...
        if name == 'tinyimagenet':
            ...
            train_val_dataset_dir = os.path.join(root, "tinyimagenet/train")
            test_dataset_dir = os.path.join(root, "tinyimagenet/val")
    ...
    if sample == 'default':
        ...
        get_train_sampler = lambda d: IdentityBatchSampler(d, kwargs['batch_size'], kwargs['num_instances'])
    ...
    trainloader = DataLoader(trainset, batch_sampler=get_train_sampler(trainset), num_workers=8)
    valloader   = DataLoader(valset,   batch_sampler=get_test_sampler(valset), num_workers=8)
